...
Для работы с torch используйте команду:

```bash
uv venv # если не создано
uv pip install torch --torch-backend=auto # ВАЖНО: запускать до sync, иначе поставится CPU версия!
uv sync # увидит что есть torch и установит остальные зависимости.
```
